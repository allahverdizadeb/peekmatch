...
const TABS: { key: Tab; label: string; minPkg: number }[] = [
  { key: 'report', label: 'Tam hesabat', minPkg: 1 },
  { key: 'cv', label: 'Uyğunlaşdırılmış CV', minPkg: 2 },
  { key: 'cover-letter', label: 'Cover letter', minPkg: 2 },
  { key: 'interview', label: 'Müsahibə hazırlığı', minPkg: 3 },
];
...
export default function Workspace() {
  ...
  if (!id || !info) {
    return (
      <div>
        <AppHeader />
        <div className="max-w-[520px] mx-auto px-6 py-20 text-center text-text2">Yüklənir...</div>
      </div>
    );
  }
      ...
      <div className="max-w-[1080px] mx-auto px-6 py-8">
        <div className="flex gap-1.5 mb-7 overflow-x-auto border-b border-border">
          {TABS.map((t) => (
            <button
              key={t.key}
              onClick={() => navigate(`/workspace/${id}/${t.key}`)}
              className={
                'px-4 py-3 text-[14px] font-semibold whitespace-nowrap border-b-2 -mb-px flex items-center gap-1.5 ' +
                (activeTab === t.key ? 'border-teal text-teal' : 'border-transparent text-text2 hover:text-navy')
              }
            >
              {owned < t.minPkg && <Lock className="w-3.5 h-3.5" />}
              {t.label}
            </button>
          ))}
        </div>
        ...
        {activeTab === 'report' &&
          (owned >= 1 ? <ReportTab id={id} /> : <LockPanel pkgName="Tam uyğunluq hesabatı" price="0.49 USD" analysisId={id} pkgId={1} />)}
        {activeTab === 'cv' &&
          (owned >= 2 ? <CvTab id={id} /> : <LockPanel pkgName="Vakansiyaya uyğun CV" price="0.99 USD" analysisId={id} pkgId={2} />)}
        {activeTab === 'cover-letter' &&
          (owned >= 2 ? <CoverLetterTab id={id} /> : <LockPanel pkgName="Cover letter" price="0.99 USD" analysisId={id} pkgId={2} />)}
        {activeTab === 'interview' &&
          (owned >= 3 ? <InterviewTab id={id} /> : <LockPanel pkgName="Müsahibə hazırlığı" price="5.90 USD" analysisId={id} pkgId={3} />)}
      </div>
    </div>
  );
}
...
function LockPanel({ pkgName, price, analysisId, pkgId }: { pkgName: string; price: string; analysisId: string; pkgId: number }) {
  ...
  return (
    <Card className="p-10 text-center">
      ...
      <h2 className="text-[18px] font-bold mb-1.5">Bu bölmə kilidlidir</h2>
      <p className="text-[14px] text-text2 mb-6">
        <span className="font-semibold text-navy">{pkgName}</span> paketini alaraq bu bölməni açın.
      </p>
...
function ReportTab({ id }: { id: string }) {
  ...
  if (!report) return <p className="text-text2">Yüklənir...</p>;
  ...
  return (
    <div className="grid gap-8">
          ...
          <table className="w-full text-[13px]">
            <thead>
              <tr className="text-left text-text2 border-b border-border">
                <th className="py-2 pr-3">Tələb</th>
                <th className="py-2 pr-3">Kateqoriya</th>
                <th className="py-2 pr-3">Vaciblik</th>
                <th className="py-2 pr-3">Status</th>
                <th className="py-2">CV-də sübut</th>
              </tr>
            </thead>
            <tbody>
              {filtered.map((r, i) => (
                <tr key={i} className="border-b border-border align-top">
                  ...
                  <td className="py-2.5 pr-3 text-text2 capitalize">{r.importance}</td>
        ...
        <div className="md:hidden grid gap-3">
          {filtered.map((r, i) => (
            <div key={i} className="border border-border rounded-rk p-3.5">
              ...
              <div className="text-[12px] text-text2 mb-1">{r.category} · {r.importance}</div>
      ...
      {report.weakPresentation.length > 0 && (
        <Card className="p-6">
          <h2 className="text-[16px] font-bold mb-4">CV-də zəif təqdim olunan təcrübələr</h2>
          <div className="grid gap-3">
            {report.weakPresentation.map((w, i) => (
              <div key={i} className="border border-border rounded-rk p-4">
                <div className="text-[12px] font-semibold text-muted mb-1">Orijinal</div>
                <p className="text-[13.5px] mb-2 italic text-text2">"{w.original}"</p>
                <div className="text-[12px] font-semibold text-warning mb-1">Problem</div>
                <p className="text-[13.5px] mb-2 text-text2">{w.issue}</p>
                <div className="text-[12px] font-semibold text-success mb-1">Tövsiyə olunan istiqamət</div>
                <p className="text-[13.5px] text-text2">{w.suggestion}</p>
              </div>
            ))}
          </div>
          <p className="text-[12px] text-muted mt-3">Platforma CV-də olmayan təcrübə və nəticələr əlavə etmir.</p>
        </Card>
      )}

      {report.improvementOpportunities.length > 0 && (
        <Card className="p-6">
          <h2 className="text-[16px] font-bold mb-4">Təkmilləşdirmə imkanları</h2>
      ...
      <Card className="p-6 flex items-center justify-between flex-wrap gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="font-semibold text-[14.5px]">Uyğunluq_Hesabatı.pdf</div>
            <div className="text-[12px] text-text2">PDF hesabat</div>
          </div>
        </div>
        <a href={`/api/analyses/${id}/report/download`} target="_blank" rel="noreferrer">
          <Button variant="secondary"><Download className="w-4 h-4" />Yüklə</Button>
        </a>
      </Card>
    </div>
  );
...
function CvTab({ id }: { id: string }) {
  ...
  if (!cv) return <p className="text-text2">Yüklənir (CV hazırlanır)...</p>;
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-[1fr_300px] gap-6">
      <Card className="p-8 font-serif">
        ...
        <h2 className="text-[14px] font-bold text-teal border-b border-border pb-1 mb-2">Peşəkar xülasə</h2>
        <p className="text-[13.5px] leading-relaxed mb-5">{cv.summary}</p>
        <h2 className="text-[14px] font-bold text-teal border-b border-border pb-1 mb-2">Əsas bacarıqlar</h2>
        <p className="text-[13.5px] leading-relaxed mb-5">{cv.skills.join(' · ')}</p>
        <h2 className="text-[14px] font-bold text-teal border-b border-border pb-1 mb-2">İş təcrübəsi</h2>
        ...
        <h2 className="text-[14px] font-bold text-teal border-b border-border pb-1 mb-2">Təhsil</h2>
        <p className="text-[13.5px] mb-5">{cv.education}</p>
        <h2 className="text-[14px] font-bold text-teal border-b border-border pb-1 mb-2">Sertifikatlar</h2>
        <p className="text-[13.5px] mb-5">{cv.certifications}</p>
        <h2 className="text-[14px] font-bold text-teal border-b border-border pb-1 mb-2">Dil bilikləri</h2>
        <p className="text-[13.5px]">{cv.languages}</p>
      </Card>

      <div className="grid gap-4 content-start">
        <Card className="p-5">
          <h3 className="text-[14px] font-bold mb-3">Nə dəyişdi?</h3>
        ...
        <a href={`/api/analyses/${id}/cv/download?format=docx`} target="_blank" rel="noreferrer">
          <Button variant="secondary" className="w-full"><Download className="w-4 h-4" />CV-ni Word formatında yüklə</Button>
        </a>
        <a href={`/api/analyses/${id}/cv/download?format=pdf`} target="_blank" rel="noreferrer">
          <Button variant="secondary" className="w-full"><Download className="w-4 h-4" />CV-ni PDF formatında yüklə</Button>
        </a>
      </div>
    </div>
  );
...
function CoverLetterTab({ id }: { id: string }) {
  ...
  if (!letter) return <p className="text-text2">Yüklənir (cover letter hazırlanır)...</p>;
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-[1fr_300px] gap-6">
      <Card className="p-8">
        <h1 className="text-[19px] font-bold mb-5">Vakansiyaya uyğun cover letter</h1>
      ...
      <div className="grid gap-4 content-start">
        <Card className="p-5">
          <h3 className="text-[14px] font-bold mb-3">Bu məktub nəyə əsaslanır?</h3>
          <ul className="grid gap-2">
        ...
        <a href={`/api/analyses/${id}/cover-letter/download`} target="_blank" rel="noreferrer">
          <Button variant="secondary" className="w-full"><Download className="w-4 h-4" />Word formatında yüklə</Button>
        </a>
        <Button
          variant="secondary"
          onClick={() => {
...
function InterviewTab({ id }: { id: string }) {
  ...
  if (!prep) return <p className="text-text2">Yüklənir (müsahibə hazırlığı hazırlanır)...</p>;

  const sections = [
    { key: 'hr', label: 'HR sualları', items: prep.hrQuestions },
    { key: 'situational', label: 'Situasiya sualları', items: prep.situational },
    { key: 'technical', label: 'Texniki suallar', items: prep.technical },
  ];
  ...
  return (
    <div className="grid gap-6">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <Card className="p-5">
          <div className="text-[12px] font-semibold text-muted mb-1">Ən güclü mövzu</div>
          <div className="text-[15px] font-bold">{prep.strongestTopic}</div>
        </Card>
        <Card className="p-5">
          <div className="text-[12px] font-semibold text-muted mb-1">Ən vacib risk</div>
      ...
      <Card className="p-6 bg-success-bg border-success/40">
        <div className="flex items-center justify-between mb-2">
          <h2 className="text-[15px] font-bold">"Tell me about yourself"</h2>
          <button
        ...
        <div className="flex items-center gap-1.5 text-[12px] text-text2 mt-3">
          <Clock className="w-3.5 h-3.5" /> ~60-90 saniyə
        </div>
      </Card>
            ...
            <div className="px-6 pb-6 grid gap-3">
              {s.items.map((q, i) => (
                <div key={i} className="border border-border rounded-rk p-4">
                  ...
                  <div className="text-[12px] font-semibold text-muted mb-1">Niyə soruşula bilər</div>
                  <p className="text-[13px] text-text2 mb-2">{q.why}</p>
                  <div className="text-[12px] font-semibold text-muted mb-1">Cavab çərçivəsi</div>
      ...
      {prep.gapExplanations.length > 0 && (
        <Card className="p-6">
          <h2 className="text-[15px] font-bold mb-3">Kritik boşluqların izahı</h2>
          <ul className="grid gap-2">
      ...
      {prep.questionsToAsk.length > 0 && (
        <Card className="p-6">
          <h2 className="text-[15px] font-bold mb-3">Müsahibəçiyə veriləcək suallar</h2>
          <ul className="grid gap-2">
